...
此清单用于公开或更新前的复核，不会自动创建仓库、提交或上传文件。整理文档不代表对应版本已经验证。

## 作者需要确认

- [x] 仓库名称和归属账号：`YunsongHong/HYS_LLM`；
- [x] 可见性：作者于 2026-08-28 选择公开；更新代码不改变此设置；
- [ ] 对外许可证尚未选择；确定后再添加根目录 LICENSE 和相符的包元数据；
- [ ] 私密漏洞接收渠道，并实际测试可用性。
